...
class Qnet(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        # LSTM 레이어 추가
        self.lstm = nn.LSTM(input_size=state_size, hidden_size=64, num_layers=1, batch_first=True)
        # Fully Connected Layer
        self.fc1 = nn.Linear(64, 64)
        self.fc2 = nn.Linear(64, action_size)

    def forward(self, x):  #GPT 사용한 함수
        # LSTM 레이어를 통과
        x = x.unsqueeze(1)  # (batch_size, seq_len=1, state_size) 형태로 변환 (LSTM 입력을 위해) 
        x, _ = self.lstm(x)
    ...
    def sample_action(self, obs, epsilon, valid_actions):
        ...
        if coin < epsilon:
            ...
            return valid_actions[out[0, valid_actions].argmax().item()] # GPT사용
...
def train_dqn(n_episodes=N_EPISODES, state_type='count', state_interval=STATE_INTERVAL, action_type='job_type', rule_list=None):
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            state_tensor = state.unsqueeze(0)  # 이미 텐서로 변환되었으므로 바로 사용 -> GPT 사용
            valid_actions = env.get_valid_actions()
            action = q.sample_action(state_tensor, epsilon, valid_actions)
            next_state, reward, done = env.step(action)
            done_mask = 0.0 if done else 1.0
            memory.put((state, action, reward / 100.0, next_state, done_mask)) # -> GPT 사용
            state = next_state
            total_reward += reward
            ...
            if memory.size() > 2000:
                ...
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(q.parameters(), max_norm=10)  # Gradient Clipping 추가
                optimizer.step()

        if episode % 500 == 0 and episode != 0:  # 더 자주 타겟 네트워크 업데이트
            q_target.load_state_dict(q.state_dict())
