...
class Qnet(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        self.fc1 = nn.Linear(state_size, 64)  # 입력 크기 줄임
    ...
    def sample_action(self, obs, epsilon, valid_actions):
        obs = obs.to(device)  # obs를 GPU 또는 MPS로 이동
...
class JobScheduler:
    ...
    def get_state(self):
        if self.state_type == 'count':
            return torch.tensor(list(self.remaining_jobs.values()), dtype=torch.float32, device=device)  # 수정된 부분: 텐서로 변환
            ...
            return torch.tensor([current_state], dtype=torch.float32, device=device)  # 수정된 부분: 텐서로 변환
