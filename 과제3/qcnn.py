...
class QnetWithCNN(nn.Module):
    def __init__(self, state_size, action_size):
        super(QnetWithCNN, self).__init__()
        # CNN 레이어 추가
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1)
        
        # Fully Connected Layer
        self.fc1 = nn.Linear(32 * state_size, 128)  # Conv 레이어의 출력 크기를 고려해 설정
        self.fc2 = nn.Linear(128, 128)
        self.fc3 = nn.Linear(128, action_size)

    def forward(self, x):
        # CNN 레이어를 통과
        x = x.unsqueeze(1)  # (batch_size, 1, state_size) 형태로 변환 (CNN 입력을 위해)
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        
        # Flatten 후 Fully Connected Layer 통과
        x = x.view(x.size(0), -1)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
...
def train_dqn(n_episodes=N_EPISODES, state_type='count', state_interval=STATE_INTERVAL, action_type='job_type', rule_list=None):
    ...
    q = QnetWithCNN(state_size, action_size)
    q_target = QnetWithCNN(state_size, action_size)
